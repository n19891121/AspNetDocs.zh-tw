...
[下載代碼](iteration-5-create-unit-tests-vb/_static/contactmanager_5_vb1.zip)
...
3. 解壓縮下載。
4. 通過選擇選單選項 **「專案」,添加引用**以打開 **「添加參考」** 對話方塊,將對 Moq 程式集的引用添加到測試專案。 在「瀏覽」選項卡下,瀏覽到解壓縮 Moq 的資料夾,然後選擇 Moq.dll 程式集。 按下 **「確定」** 按鈕(參見圖 2)。

[![解除阻止莫克](iteration-5-create-unit-tests-vb/_static/image1.jpg)](iteration-5-create-unit-tests-vb/_static/image1.png)

**圖 01**: 取消封鎖 Moq([按下以檢視全尺寸影像](iteration-5-create-unit-tests-vb/_static/image2.png))

[![新增 Moq 後的參考](iteration-5-create-unit-tests-vb/_static/image2.jpg)](iteration-5-create-unit-tests-vb/_static/image3.png)

**圖 02**: 新增 Moq([按下以檢視全尺寸影像](iteration-5-create-unit-tests-vb/_static/image4.png))後的參考

## <a name="creating-unit-tests-for-the-service-layer"></a>為服務層建立單元測試

讓我們首先為聯繫人管理器應用程式的服務層創建一組單元測試。 我們將使用這些測試來驗證我們的驗證邏輯。

在 ContactManager.測試項目中創建名為" 模型" 的新資料夾。 接下來,右鍵按兩下"模型"資料夾並選擇 **「添加、新建測試**」。 將顯示圖 3 所示**的「添加新測試**」對話方塊。 選擇**單元測試**模板並命名新的測試連絡人管理員服務測試.vb。 按下 **「確定」** 按鈕將新測試添加到測試專案。

> [!NOTE] 
> 
> 通常,您希望測試項目的資料夾結構與 ASP.NET MVC 專案的資料夾結構匹配。 例如,將控制器測試放在控制器資料夾中,在模型資料夾中放置模型測試,等等。

[![型號_連絡管理員服務測試.cs](iteration-5-create-unit-tests-vb/_static/image3.jpg)](iteration-5-create-unit-tests-vb/_static/image5.png)

**圖 03**:型號_ContactManagerServiceTest.cs([按下以檢視全尺寸影像](iteration-5-create-unit-tests-vb/_static/image6.png))
...
> [!NOTE] 
> 
> 使用 Moq 框架時,mockRepository 和\_\_mockRepository.Object 區別是有區別的。 前者是指 Mock(IContactManager Repository)類,該類包含用於指定類比儲存庫如何執行的方法。 後者是指實現 IContactManager 儲存庫介面的實際類比儲存庫。
...
**圖 04**: 測試結果 ([按下以檢視全尺寸影像](iteration-5-create-unit-tests-vb/_static/image8.png))

## <a name="creating-unit-tests-for-controllers"></a>為控制器建立單元測試

ASP.NET MVC 應用程式控制使用者互動流。 測試控制器時,要測試控制器是否返回正確的操作結果並查看數據。 您可能還希望測試控制器是否與模型類以預期的方式交互。
...
> [前一個](iteration-4-make-the-application-loosely-coupled-vb.md)
> [下一個](iteration-6-use-test-driven-development-vb.md)
